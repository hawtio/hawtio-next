...
import { HOME, INDEX, PREFERENCES } from '@hawtiosrc/RouteConstants'
  ...
  return (
    <React.Fragment>
        ...
        <Nav aria-label='Nav' variant='tertiary'>
          <NavList>
            {preferencesRegistry.getPreferences().map(prefs => (
              <NavItem key={prefs.id} isActive={hawtio.fullPath(location.pathname) === hawtio.fullPath(PREFERENCES, prefs.id)}>
      ...
      <PageSection variant={PageSectionVariants.light}>
        <Switch>
          {preferencesRegistry.getPreferences().map(prefs => (
            <Route key={prefs.id} path={hawtio.fullPath(PREFERENCES, prefs.id)}>{React.createElement(prefs.component)}</Route>
          ))}
          <Route exact path={hawtio.fullPath(PREFERENCES)}><Redirect to={hawtio.fullPath(PREFERENCES, HOME)} /></Route>
        </Switch>
      </PageSection>
    </React.Fragment>
  )
